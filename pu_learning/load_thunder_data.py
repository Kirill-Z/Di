...
class ThunderData(Data):
    ...
    def get_data(self):
        ...
        read_file.close()
        return self.convert_str_to_float(true_data), self.convert_str_to_float(false_data)

    def convert_str_to_float(self, data):
    ...
    def separate_false_and_true_data(self):
        ...
        for file in os.listdir(self.dir_path):
            read_file = pd.read_csv(os.path.join(self.dir_path, file), usecols=[i for i in range(91)])
            ...
            true_data.to_csv(os.path.join(self.clear_data_path, "true_data.csv"), mode="a", header=None, index=False)
            false_data.to_csv(os.path.join(self.clear_data_path, "false_data.csv"), mode="a", header=None, index=False)
